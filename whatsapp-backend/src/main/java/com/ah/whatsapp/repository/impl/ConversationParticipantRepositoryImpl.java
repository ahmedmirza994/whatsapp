...
public class ConversationParticipantRepositoryImpl implements ConversationParticipantRepository {
	...
	public ConversationParticipant save(ConversationParticipant participant) {
		...
		ConversationParticipantEntity entity =
				participantMapper.toEntity(participant, conversationEntity, userEntity);
	...
	public Optional<ConversationParticipant> findById(UUID id) {
		return participantEntityRepository.findById(id).map(participantMapper::toModel);
	}
	...
	@Override
	public boolean existsByConversationIdAndUserIdAndIsActiveTrue(
			UUID conversationId, UUID userId) {
		return participantEntityRepository.existsByConversationIdAndUserIdAndIsActiveTrue(
				conversationId, userId);
	}

	@Override
	public Map<UUID, List<ConversationParticipant>> findParticipantsForConversationsAndIsActiveTrue(
			List<UUID> conversationIds) {
		if (conversationIds == null || conversationIds.isEmpty()) {
			return Collections.emptyMap();
		}
		List<ConversationParticipantEntity> participantEntities =
				participantEntityRepository.findByConversationIdInAndIsActiveTrueWithUser(
						conversationIds);
	...
	@Override
	public Optional<ConversationParticipant> findByConversationIdAndUserIdAndIsActiveTrue(
			UUID conversationId, UUID userId) {
		return participantEntityRepository
	...
	@Override
	public Optional<ConversationParticipant> findByConversationIdAndUserId(
			UUID conversationId, UUID userId) {
		return participantEntityRepository
