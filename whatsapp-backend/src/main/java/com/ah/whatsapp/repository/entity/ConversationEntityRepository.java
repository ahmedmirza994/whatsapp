...
public interface ConversationEntityRepository extends JpaRepository<ConversationEntity, UUID> {
  ...
  @Query(
        value= "select c.id from conversations c join conversation_participants cp on c.id = cp.conversation_id" +
        " where cp.user_id in :userIds group by c.id " +
        "having count(distinct cp.user_id) = :participantCount and count(cp.id) = :participantCount",
		nativeQuery = true
    )
