    ...
    public void handleSendMessage(@Payload @Valid SendMessageRequest request,
        ...
        if (userPrincipal instanceof UsernamePasswordAuthenticationToken authToken &&
            ...
            if (jwtUser.getUserId() == null) {
                 log.error("WebSocket user principal (JwtUser) has null fields despite being authenticated. Principal: {}", jwtUser);
                 // Handle error appropriately - maybe send error back to user?
                 return;
            }

            log.info("Received message via WebSocket from user {}: {}", jwtUser.getUserId(), request);
