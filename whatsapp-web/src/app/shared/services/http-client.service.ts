...
export class HttpClientService {
	...
	get<T>(url: string, options?: { headers?: HttpHeaders; params?: HttpParams }): Observable<ApiResponse<T>> {
	...
	post<T>(url: string, body: unknown, options?: { headers?: HttpHeaders }): Observable<ApiResponse<T>> {
	...
	put<T>(url: string, body: unknown, options?: { headers?: HttpHeaders }): Observable<ApiResponse<T>> {
	...
	private preprocessRequestBody(body: unknown): unknown {
		if (body && typeof body === 'object' && !(body instanceof FormData)) {
			return Object.keys(body as Record<string, unknown>).reduce((acc: { [key: string]: unknown }, key) => {
				const typedBody = body as Record<string, unknown>;
				acc[key] = typedBody[key] === '' ? null : typedBody[key];
				return acc;
			}, {});
		}
		return body;
