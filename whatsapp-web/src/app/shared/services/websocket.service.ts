...
export class WebSocketService implements OnDestroy {
	...
	private connect(): void {
		if (
			...
			this.connectionState() === ConnectionState.ATTEMPTING
		) {
	...
	public subscribeToTopic(destination: string): void {
		...
		if (this.isConnected()) {
			...
			console.log(
				`WebSocket: Connection not ready for ${destination}. Queuing subscription.`
			);
	...
	private subscribeToTopicInternal(destination: string): void {
		if (!this.stompClient || !this.isConnected()) {
			console.error(
				`WebSocket: Internal error - Tried to subscribe to ${destination} while disconnected.`
			);
	...
	public unsubscribeFromTopic(destination: string): void {
		...
		if (subscription) {
			...
			console.warn(
				`WebSocket: No active subscription found for ${destination} to unsubscribe.`
			);
		}
	}
	...
	public sendMessage(destination: string, payload: any): void {
		if (!this.isConnected()) {
			// Use computed signal
			console.error(
				`WebSocket: Cannot send message to ${destination}. Client not connected.`
			);
